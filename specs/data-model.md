...
**구술로 확정**: 기본키는 숫자 별도 · 컬럼명 `username` · 유일 제약 있음

**Claude가 스펙에서 도출한 것** (구술 아님 — 이견 있으면 지적할 것):
필수 여부. 가입 시 모든 항목을 입력받으므로(`FR-001`) 전부 필수로 둔다.
...
티켓이 로그인마다 하나씩 생기므로 결과적으로 "기기마다 하나"가 되지만, **어느 기기인지는
저장하지 않는다.** 스펙에 그런 요구가 없고 `로그인 이력 조회`는 Out of Scope다.
**티켓이 여러 개 생긴다는 사실만 중요하고 어느 기기인지는 알 필요가 없다.**
...
| 질문 | 답 | |
|---|---|---|
| 유저 하나가 → 재발급 티켓 몇 개를 가지나? | **여러 개** | 구술 |
| 재발급 티켓 하나가 → 유저 몇 명에게 속하나? | **1명** | 구술 |
| → 따라서 관계는? | **1 : N** (FK는 N쪽 = 티켓 테이블에) | 구술 |
| 사용불가 아이디는 다른 테이블과 관계가 있나? | **없음. 독립 테이블** | Claude 도출 |

**"관계 없음"의 근거** (구술 아님 — 이견 있으면 지적할 것):
금지 아이디 목록의 한 줄은 **특정 유저를 가리키지 않는다.** 오히려 **유저가 되지 못하게
막는 목록**이다. 가입 시 "이 값이 목록에 있나?"를 조회만 하고, 두 테이블의 행이 서로를
참조하지 않으므로 외래키가 생기지 않는다.
